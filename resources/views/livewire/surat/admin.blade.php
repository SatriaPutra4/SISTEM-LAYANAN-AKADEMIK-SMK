<div class="space-y-6">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <div class="text-sm font-bold text-gray-800 dark:text-gray-200">{{ $s->siswa->user->name }}</div>
                                    <div class="text-xs text-gray-500">NIS: {{ $s->siswa->nis }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2.5 py-1 text-xs font-bold rounded-full 
                                {{ $s->status === 'Disetujui' ? 'bg-green-100 text-green-700' : ($s->status === 'Diproses' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700') }}">
                                {{ $s->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <button wire:click="showVerifikasi({{ $s->id }})" class="text-primary-600 hover:text-primary-700 font-bold text-sm flex items-center">
    ...
    <!-- Modal Verifikasi -->
    <x-modal name="modal-verifikasi-surat" title="Detail & Verifikasi Surat">
        @if($selectedSurat)
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="space-y-1">
                        <p class="text-sm font-bold text-gray-800 dark:text-white">{{ $selectedSurat->siswa->user->name }}</p>
                        <p class="text-xs text-gray-500">NIS: {{ $selectedSurat->siswa->nis }}</p>
                        <p class="text-xs text-gray-500">{{ $selectedSurat->siswa->kelas->nama_kelas }} - {{ $selectedSurat->siswa->jurusan->nama_jurusan }}</p>
                    </div>
                </div>
                <div>
                    <h4 class="text-xs font-bold text-gray-400 uppercase mb-2">Detail Pengajuan</h4>
                    <div class="space-y-1">
                        <p class="text-sm font-bold text-gray-800 dark:text-white">{{ $selectedSurat->jenis_surat }}</p>
                        <p class="text-xs text-gray-500">Diajukan: {{ $selectedSurat->created_at->format('d M Y, H:i') }}</p>
                    </div>
                </div>
            </div>
            ...
            @if($selectedSurat->dokumen_pendukung)
            <div>
                <h4 class="text-xs font-bold text-gray-400 uppercase mb-2">Dokumen Pendukung</h4>
                <a href="{{ asset('storage/' . $selectedSurat->dokumen_pendukung) }}" target="_blank" class="inline-flex items-center text-primary-600 hover:underline text-sm font-medium">
