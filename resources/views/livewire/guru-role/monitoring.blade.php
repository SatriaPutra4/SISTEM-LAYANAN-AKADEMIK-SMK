<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <p class="text-3xl font-bold text-primary-600 mt-1">{{ number_format($avgNilai, 2) }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 border-l-4 border-l-emerald-500">
            ...
            <p class="text-3xl font-bold text-emerald-600 mt-1">{{ $tuntasCount }}</p>
            <div class="mt-4 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full bg-emerald-500" style="width: {{ ($tuntasCount + $remedialCount) > 0 ? ($tuntasCount / ($tuntasCount + $remedialCount) * 100) : 0 }}%"></div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 border-l-4 border-l-red-500">
            ...
            <p class="text-3xl font-bold text-red-600 mt-1">{{ $remedialCount }}</p>
            <div class="mt-4 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full bg-red-500" style="width: {{ ($tuntasCount + $remedialCount) > 0 ? ($remedialCount / ($tuntasCount + $remedialCount) * 100) : 0 }}%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-6">
                <div class="space-y-4">
                    @forelse($siswaBermasalah as $nilai)
                        <div class="flex items-center justify-between p-3 bg-red-50/50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/40 flex items-center justify-center text-red-600 font-bold text-xs">
                                    {{ substr($nilai->siswa->user->name, 0, 1) }}
                                </div>
                                <div>
                                    <p class="text-sm font-bold text-gray-800 dark:text-white">{{ $nilai->siswa->user->name }}</p>
                                    <p class="text-[10px] text-gray-500 uppercase">{{ $nilai->siswa->kelas->nama_kelas }} | {{ $nilai->mataPelajaran->nama_mapel }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-black text-red-600">{{ number_format($nilai->nilai_akhir, 2) }}</p>
                            </div>
                        </div>
                    @empty
