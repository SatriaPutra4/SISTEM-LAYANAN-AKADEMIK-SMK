                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <span class="px-2 py-1 bg-primary-50 dark:bg-primary-900/20 text-primary-600 text-[10px] font-bold rounded uppercase tracking-wider">
                            {{ $p->target_role === 'all' ? 'Untuk Umum' : 'Khusus Guru' }}
                        </span>
                        <span class="text-xs text-gray-400 font-medium">{{ $p->created_at->translatedFormat('d F Y') }}</span>
                    </div>
                    <h3 class="text-lg font-bold text-gray-800 dark:text-white mb-2">{{ $p->judul }}</h3>
                    ...
                    <div class="mt-6 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <div class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-[10px] font-bold text-gray-500">
                                {{ substr($p->author->name ?? 'A', 0, 1) }}
                            </div>
                            <span class="text-xs font-medium text-gray-500">{{ $p->author->name ?? 'Admin' }}</span>
                        </div>
