<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('guru-role.nilai.index') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ $totalMapel }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ $jadwalHariIni->count() }} Sesi</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    ...
                    <span class="text-xs font-medium text-primary-600 bg-primary-50 dark:bg-primary-900/20 px-2 py-1 rounded-full">{{ now()->translatedFormat('l, d F Y') }}</span>
                </div>
                            ...
                            @forelse($jadwalHariIni as $jadwal)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-800 dark:text-gray-300">{{ $jadwal->mataPelajaran->nama_mapel }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-800 dark:text-gray-300">{{ $jadwal->kelas->nama_kelas }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
        ...
        <div class="space-y-6">
                    ...
                    @forelse($pengumumanTerbaru as $pengumuman)
                        <div class="flex gap-4 group cursor-pointer">
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="text-sm font-semibold text-gray-800 dark:text-white truncate">{{ $pengumuman->judul }}</h4>
                                <p class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mt-1">{{ strip_tags($pengumuman->konten) }}</p>
                                <span class="text-[10px] font-medium text-gray-400 mt-2 block">{{ $pengumuman->created_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    @empty
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <a href="{{ route('guru-role.siswa') }}" class="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 transition-all group">
                    ...
                    <a href="{{ route('guru-role.jadwal') }}" class="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 transition-all group">
                    ...
                    <a href="{{ route('guru-role.monitoring') }}" class="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 transition-all group">
                    ...
                    <a href="{{ route('guru-role.profil') }}" class="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 transition-all group">
        ...
        document.addEventListener('livewire:navigated', () => {
            ...
            if (ctx) {
                new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: @json($statsNilai->pluck('mataPelajaran.nama_mapel')),
                        datasets: [{
                            label: 'Rata-rata Nilai',
                            data: @json($statsNilai->pluck('average')),
